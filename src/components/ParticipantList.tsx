    ...
    <table className="border-2 border-red-900 w-full max-w-[800px] bg-amber-50 p-4 border-collapse border border-gray-400 text-red-900 w-full table-auto">
      <thead className="bg-amber-100">
        <tr className="">
          ...
          <th className="p-2 text-center font-bold">DETAILS</th>
        </tr>
      </thead>
      <tbody>
        {participants.map((p, index) => (
          <tr
            key={p.id}
            className={`border-2 border-red-900 ${index % 2 ? "bg-amber-100" : "bg-amber-50"}`}
          >
            <td className="p-2 text-center">{index + 1}</td>
            <td className="p-2 font-semibold">
              <div className="grid">
                <span className="uppercase text-center my-2 border-b border-red-900">
                  {p.name}
                </span>
                <StyledButton
                  type="button"
            ...
            <td className="p-2">
              <div className="space-y-1 text-center">
                <p>
                  ...
                  <span className="font-bold">{p.initiative}</span>
                </p>
                <p className="flex items-center justify-center gap-2 text-center">
                  <StyledButton
                    type="button"
                    onClick={() => decreaseHP(p.id)}
                    className="px-3 rounded-full font-bold text-sm bg-red-900 hover:bg-red-800"
                  >
                    -
                  </StyledButton>
                  <span>HP: </span>
                  <span className="font-bold">{p.hp}</span>
                  <StyledButton
                    type="button"
                    onClick={() => increaseHP(p.id)}
                    className="px-3 rounded-full font-bold text-sm bg-green-900 hover:bg-green-800"
                  >
                    +
                  </StyledButton>
                </p>
                <p>
                  <span>AC: </span>
                  <span className="font-bold">{p.ac}</span>
                </p>
              </div>
            </td>
          </tr>
